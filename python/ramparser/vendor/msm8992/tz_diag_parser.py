...
# 08/04/14   abirudur  Added new aarch64 structure map for 8-byte alignment
# 09/10/14   abirudur  Added support for LE aarch64 [msm8939]
# 10/13/14   abirudur  Added support for MDM 9x25
# 02/12/14   abirudur  Added support for MSM 8992
#                      [ New Wakeup Info Parameter Added to diag region ]
#===============================================================================
...
class TZ_BSP_Dump() :       
    def __init__ (self,                  
        ...
        self.tzbsp_diag_struct_boot_map_aarch64 = []
        self.tzbsp_diag_struct_boot_map_aarch64_LE = []
        self.tzbsp_diag_struct_wakeup_info_map     = []
    ...
    def createSymbolMap(self,symbol) :
        if symbol in "g_tzbsp_diag":             
             ...
             self.tzbsp_diag_struct_boot_map_aarch64_8992   = \
                     [["Warm Entry",'I'],    #1
                      ["Warm Exit",'I'],     #2
                      ["PC Entry",'I'],      #3
                      ["PC Exit",'I'],       #4                      
                      ["WarmJump Addr",'Q'], #5
                      ["WarmJump Inst",'I'],
                      ["Dummy",'I']]         #0  < Dummy pad for 8 byte alignment] #6

             self.tzbsp_diag_struct_boot_map_aarch64   = \
                     [["Dummy",'I'],         #0  < Dummy pad for 8 byte alignment
                      ["Warm Entry",'I'],    #1
                      ["Warm Exit",'I'],     #2
                      ["PC Entry",'I'],      #3
                      ["PC Exit",'I'],       #4                      
    ...
    def processDiagData(self,offset) :        
        try:            
            ...
            if  self.tzbsp_diag_struct_general_map[2][2] == 4 \
                or  self.tzbsp_diag_struct_general_map[2][2] == 2 \
                or  self.tzbsp_diag_struct_general_map[2][2] == 1:
                l_unpack_info = build_unpack_structure_string(self.tzbsp_diag_struct_boot_map,
                                                          1,
                                                          self.tzbsp_diag_struct_general_map[2][2])
            elif self.tzbsp_diag_struct_general_map[2][2] == 8 \
                or  self.tzbsp_diag_struct_general_map[2][2] == 6:
                if l_file_size > _512KB and self.tzbsp_diag_struct_general_map[2][2] != 6:
                  l_unpack_info = build_unpack_structure_string(self.tzbsp_diag_struct_boot_map_aarch64_LE,
                                                          1,
                                                          self.tzbsp_diag_struct_general_map[2][2])
                if l_file_size > _512KB and self.tzbsp_diag_struct_general_map[2][2] == 6:
                  l_unpack_info = build_unpack_structure_string(self.tzbsp_diag_struct_boot_map_aarch64_8992,
                                                          1,
                                                          self.tzbsp_diag_struct_general_map[2][2])
                else:
                  l_unpack_info = build_unpack_structure_string(self.tzbsp_diag_struct_boot_map_aarch64,
                                                          1,
                                                          self.tzbsp_diag_struct_general_map[2][2])            
            ...
            #     HEADER
            # ==================
            display_inline_banner("BOOT STATUS",41,"-")
            display_line(102,"-","+")
            log(" + %-4s | %-8s | %-10s | %-10s | %-10s | %-18s | %-18s +" %("CPU",
            ...
            if self.tzbsp_diag_struct_general_map[2][2] == 8 or self.tzbsp_diag_struct_general_map[2][2] == 6:                
              l_index_offset = 7
            else:
              l_index_offset = 6             
            ...
            #      CONTENTS
            # ==================
            for index in range(0,len(raw_data),l_index_offset):
                if self.tzbsp_diag_struct_general_map[2][2] == 8:
                   index += 1 # <-- key dummy pad for alignment
                log(" + %-4d | 0x%08x | 0x%08x | 0x%08x | 0x%08x | 0x%016x | 0x%016x +"
                    %(l_loop_index,
                      raw_data[index],
                      raw_data[index + 1],
                      raw_data[index + 2],
            ...
            if self.tzbsp_diag_struct_general_map[2][2] == 8 and l_file_size <= _512KB:
               bin_file_offset = bin_file.tell()
               bin_file_offset += 0x4 # <-- Increment for 8 byte alignment               
               bin_file.seek(bin_file_offset)
            ...
            # ==================
            if self.tzbsp_diag_struct_general_map[2][2] == 8:
              display_inline_banner("INTERRUPT INFO",85,"-")
              display_line(197,"-","+")
            elif self.tzbsp_diag_struct_general_map[2][2] == 6:
              display_inline_banner("INTERRUPT INFO",72,"-")
              display_line(171,"-","+")
            elif self.tzbsp_diag_struct_general_map[2][2] == 1:
              display_inline_banner("INTERRUPT INFO",39,"-")
              display_line(106,"-","+")
            else:
            ...
            if self.tzbsp_diag_struct_general_map[2][2] == 8:
                log(" + %-6s | %-10s | %-10s | %-10s | %-10s | %-15s  "\
                    "| IntrCnts => %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s +"
                    %("INDEX",
                ...
                log(" + %-6s | %-10s | %-10s | %-10s | %-10s | %-15s  "\
                    "| IntrCnts => %-10s | %-10s | %-10s | %-10s | %-10s | %-10s +"
                    %("INDEX",
                      self.tzbsp_diag_struct_int_map[0][0],
                      self.tzbsp_diag_struct_int_map[1][0],
                      self.tzbsp_diag_struct_int_map[2][0],
                      ...
                      "CPU4",
                      "CPU5"))                
            elif self.tzbsp_diag_struct_general_map[2][2] == 4:
                log(" + %-6s | %-10s | %-10s | %-10s | %-10s | %-15s  "\
            ...
            # ==================
            if self.tzbsp_diag_struct_general_map[2][2] == 8:
                for index in range(0,len(raw_data),13):
                ...
                for index in range(0,len(raw_data),11):
                    l_string = str(raw_data[index + 4]).decode('ascii',errors='ignore')
                    l_entry  = ''
            ...
            l_unpack_info = build_unpack_structure_string(self.tzbsp_diag_struct_wakeup_info_map,
                                                          1,
                                                          self.tzbsp_diag_struct_general_map[2][2])
            ...
            #    WAKE UP INFO HEADER
            # ==========================            
            display_inline_banner("WAKE UP INFO",8,"-")
            display_line(40,"-","+")
            log(" + %-8s | %-10s | %-12s +" %("CPU",
                                            self.tzbsp_diag_struct_wakeup_info_map[0][0],
                                            self.tzbsp_diag_struct_wakeup_info_map[1][0]))
            ...
            # ==================
            if self.tzbsp_diag_struct_general_map[2][2] == 8:
              display_inline_banner("TZBSP LOG [ RING BUFFER ]",70,"=")
            else:
              display_inline_banner("TZBSP LOG [ RING BUFFER ]",40,"=")
            
            if self.tzbsp_diag_struct_general_map[2][2]    == 8 \
               or self.tzbsp_diag_struct_general_map[2][2] == 6:  # To be done
              l_log_warp   = bin_file.read(H_WORD)
              l_log_offset = bin_file.read(H_WORD)
              l_log_offset = unpack("<H",l_log_offset) # next write position

            # ==================
            #      CONTENTS
            # ==================
            self.ring_buffer_len = self.tzbsp_diag_struct_general_map[8][2]
            byteRead = (bin_file.read(self.ring_buffer_len)).decode('ascii',errors='ignore')  
            byteRead = byteRead.replace("\r\n","\n") # Line De-limiter 
            byteRead = byteRead.replace("\0","") # Remove NULL to display only log content
    ...
    def forceCheckTZDiagMG(self) :
        ...
        if self.l_log_binary != "CHECK_BOTH" and self.l_brute_force == 1:
            ...
            if( l_file_size > _512KB):
                l_limit_search = l_file_size
                self.tz_base_offset = 0x00100000
            else:
                l_limit_search = l_file_size
                self.tz_base_offset = 0
            ...
            while l_offset < (self.tz_base_offset + l_limit_search):              
               ...
               if value[0] == TZBSP_DIAG_MAGIC_NUM:
                  l_status = 1
                  break 
               if l_offset == 0x00200000 and (self.tz_base_offset + l_limit_search) \
                           > 0x06000000 :         # If not MDM then MSM
                  l_offset =  0x06E00000 - _4KB   # Possible MSM offset base        
                       ...
                       if l_file_size >= l_offset_entry:
                           ...
                           if (value[0] & 0xfe000000) == 0xfe000000:   #B                               
                               ...
                               break
                                ...
                                break
                           elif (value[0] & 0x6D00000) == 0x6D00000:  #8992                                 
                                self.diag_offset = value[0]
                                l_offset = value[0] & 0xFFFFFFF
                                break
...
def check_mandatory_fields(p_ocimem_file_name,p_ebi_cs0_file_name):
    ...
    if p_ebi_cs0_file_name == "DEFAULT" and l_ocimem_file_size <= _512KB \
                                        and l_ocimem_file_size >= _232KB:
        p_ebi_cs0_file_name = p_ocimem_file_name
        
    if( l_ocimem_file_size > _512KB):
...
log("\n ===============================================")
log(" | Total Processing Time :: %g seconds |"  %(l_processing_end - l_processing_start))
log(" ===============================================")
